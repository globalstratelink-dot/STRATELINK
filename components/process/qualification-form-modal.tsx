...
export function QualificationFormModal() {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent
        className={cn(
          "max-w-lg lg:max-w-xl w-[92vw] max-h-[96vh] min-h-[640px] bg-navy border border-white/15 p-0 gap-0 text-white sm:rounded-lg flex flex-col overflow-hidden",
          "[&>button]:text-gray-300 [&>button]:border [&>button]:border-white/20 [&>button]:rounded-sm [&>button]:p-1.5 [&>button]:top-4 [&>button]:right-4 [&>button]:opacity-100"
        )}
      >
        <DialogTitle className="sr-only">{t("qualFormTitle")}</DialogTitle>

        <div className={cn("border-b border-white/10 shrink-0", isCompactStep ? "px-5 pt-5 pb-3" : "px-6 sm:px-8 pt-6 sm:pt-8 pb-4")}>
          <p className="text-base sm:text-lg tracking-[0.2em] sm:tracking-[0.25em] text-copper uppercase font-medium pr-10">
            {t("qualFormTitle")}
          </p>
          <div className={cn("border-t border-white/15", isCompactStep ? "mt-3 pt-3" : "mt-4 sm:mt-5 pt-4 sm:pt-5")}>
        ...
        <div
          className={cn(
            isCompactStep
              ? "px-5 py-4 overflow-hidden flex-1 min-h-0"
              : isReview
                ? "px-6 sm:px-8 py-6 overflow-y-auto scrollbar-form flex-1 min-h-0"
                : "px-6 sm:px-8 py-8 overflow-y-auto scrollbar-hide flex-1 min-h-0"
          )}
        >
          {submitted ? (
            ...
            <>
              ...
              {step === "profile" && (
                <div className="space-y-8">
                  <div>
                    <h3 className="text-3xl sm:text-4xl font-bold text-white mb-2">{t("qualProfileTitle")}</h3>
              ...
              {step === "opportunity" && (
                <div className="space-y-8">
                  <div>
                    <h3 className="text-3xl sm:text-4xl font-bold text-white mb-2">{t("qualOpportunityTitle")}</h3>
                    <p className={descClass}>{t("qualOpportunityDesc")}</p>
                  </div>
                  <div>
                    <p className={labelClass}>{t("qualVolumeLabel")}</p>
                    <p className="text-3xl sm:text-4xl text-copper font-serif font-light text-center mb-6 transition-all duration-150">
                  ...
                  <div>
                    ...
                    <label className="flex flex-col items-center justify-center border border-dashed border-white/20 rounded-sm p-8 cursor-pointer hover:border-copper/40 transition-colors">
              ...
              {step === "review" && (
                <div className="space-y-6">
                  <div>
                    <h3 className="text-3xl sm:text-4xl font-bold text-white mb-2">{t("qualReviewTitle")}</h3>
                    <p className={descClass}>{t("qualReviewDesc")}</p>
                  </div>
                  <div className="border-t border-white/10">
                    {reviewRows.map((row) => (
                      <div key={row.label} className="grid grid-cols-[1fr_1.2fr] gap-4 py-4 border-b border-white/10 text-sm">
                        <span className="text-[11px] tracking-[0.15em] text-copper uppercase font-medium">{row.label}</span>
                        <span className="text-gray-200 text-right break-words">{row.value}</span>
                      </div>
                    ))}
                  </div>
                  <div className="flex gap-3 items-start border border-white/15 p-4 rounded-sm">
        ...
        {!submitted && (
          <div className={cn("border-t border-white/10 flex justify-between gap-4 shrink-0", isCompactStep ? "px-5 py-4" : "px-6 sm:px-8 py-5")}>
            {stepIndex > 0 ? (
              <Button type="button" variant="outline" onClick={prevStep} className="border-white/25 bg-transparent text-gray-200 hover:bg-white/5 uppercase tracking-wider text-xs">
                <ArrowLeft className="w-4 h-4 mr-2" />{t("qualBack")}
              </Button>
            ) : (<div />)}
            {step === "review" ? (
              <Button type="button" onClick={handleSubmit} disabled={!form.consent || submitting} className="bg-copper hover:bg-copper/90 text-navy font-bold uppercase tracking-wider text-xs px-6">
                {submitting ? t("qualSubmitting") : t("qualSubmit")}<ArrowRight className="w-4 h-4 ml-2" />
              </Button>
            ) : (
              <Button type="button" onClick={nextStep} className="bg-copper hover:bg-copper/90 text-navy font-bold uppercase tracking-wider text-xs px-6">
                {step === "opportunity" ? t("qualReviewBtn") : t("qualContinue")}<ArrowRight className="w-4 h-4 ml-2" />
              </Button>
            )}
          </div>
        )}
...
function SelectCard({ selected, onClick, title, description }: { selected: boolean; onClick: () => void; title: string; description: string }) {
  return (
    <button type="button" onClick={onClick} className={cn("flex gap-3 text-left p-4 border rounded-sm transition-colors", selected ? "border-copper bg-copper/5" : "border-white/15 hover:border-white/30")}>
  ...
  return (
    <button
      ...
      className={cn(
        ...
        compact ? "gap-2 p-2.5 items-start" : "gap-3 p-4",
